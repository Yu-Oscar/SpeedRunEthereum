...
contract MyUSDEngine is Ownable {
    ...
    function getCurrentDebtValue(address user) public view returns (uint256) {
        uint256 share = s_userDebtShares[user];
        if (share == 0) return 0;

        return (share * _getCurrentExchangeRate()) / PRECISION;
    }

    function calculatePositionRatio(address user) public view returns (uint256) {
        uint256 debt = getCurrentDebtValue(user);
        uint256 collateral = calculateCollateralValue(user);
    ...
    function _validatePosition(address user) internal view {
        uint256 pos = calculatePositionRatio(user);
        if ((pos * 100) < (COLLATERAL_RATIO * PRECISION)) revert Engine__UnsafePositionRatio();
    }
